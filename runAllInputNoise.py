...
for id in range(len(noise_list)):
	...
	for round in range(10):
		...
		N_rec = 100
		# initialize task
		task = Checkerboard2AFC(dt=dt, tau=tau, T=T, N_batch=N_batch, noise = noise_list[id])
		...
		train_params['save_weights_path'] = "./weights/basic2InputNoise" + str(noise_list[id]) + "round" + str(round)
		# train model
		losses, initialTime, trainTime = model.train(task, train_params)
		#####################################################
		## save losses
		#####################################################
		loss_name = "./losses/basic2InputNoise" + str(noise_list[id]) + "round" + str(round) + ".txt"
		...
		#############################################
		resultPath = "./resultData/basic2InputNoise" + str(noise_list[id]) + "round" + str(round) + ".csv"
