...
class AdaptiveGain(Basic2):
    """The adaptive gain continuous time recurrent neural network model.

    Slightly edited version of the Basic RNN of :class:`psychrnn.backend.models.basic.Basic`

    Basic implementation of :class:`psychrnn.backend.rnn.RNN` with a simple RNN, enabling biological constraints. This model has a separate pool of neurons which set the gain of RNN unit activity.
    ...
    def __init__(self, params):
        ...
        super(Basic2, self).__init__(params)

        # ----------------------------------
        # Network sizes (tensor dimensions)
        # ----------------------------------
        try:
            N_gain = self.N_gain = params["N_gain"]
        except KeyError:
            print("You must pass 'N_gain' to Adaptive Gain RNN")
            raise

        self.gain_transfer_function = params.get("gain_transfer_function", tf.nn.relu)

        # ----------------------------------
        # Trainable features
        # ----------------------------------

        self.init_gain_train = params.get("init_gain_train", True)
        self.W_gain_train = params.get("W_gain_train", True)
        self.b_gain_train = params.get("b_gain_train", True)
        ...
        with tf.compat.v1.variable_scope(self.name) as scope:
            ...
            try:
                self.init_gain = tf.compat.v1.get_variable(
                    "init_gain",
                    [1, N_gain],
                    initializer=self.initializer.get("init_gain"),
                    trainable=self.init_gain_train,
                )
            ...
            self.init_gain = tf.tile(self.init_gain, [self.N_batch, 1])

            self.W_gin = tf.compat.v1.get_variable(
                "W_gin",
                [N_rec, N_gain],
                initializer=self.initializer.get("W_gain"),
                trainable=self.W_gain_train,
            )
            self.W_grec = tf.compat.v1.get_variable("W_grec", [N_gain, N_gain], initial)
            self.b_gain = tf.compat.v1.get_variable(
                "b_gain",
                [N_gain],
                initializer=self.initializer.get("b_gain"),
                trainable=self.b_gain_train,
            )
            self.gain_in_connectivity = tf.compat.v1.get_variable(
                "gain_in_connectivity",
                [N_rec, N_gain],
                initializer=self.initializer.get("gain_in_connectivity"),
                trainable=False,
            )
            self.gain_out_connectivity = tf.compat.v1.get_variable(
                "gain_out_connectivity",
                [N_gain, N_rec],
                initializer=self.initializer.get("gain_out_connectivity"),
                trainable=False,
            )

    def apply_rec_gain(self, state, gain):

        rec_gain = tf.matmul()

    def gain_timestep(self, rec, gain):

        new_gain = (
            ((1 - self.alpha) * gain)
            + self.alpha
            * (
                tf.matmul(gain, self.W_grec, transpose_b=True, name="3")
                + tf.matmul(rec, self.W_gin, transpose_b=True, name="4")
                + self.b_gain
            )
            + tf.sqrt(2.0 * self.alpha * self.gain_noise * self.gain_noise)
            * tf.random.normal(tf.shape(input=gain), mean=0.0, stdev=1.0)
        )

        new_gain = self.gain_transfer_function(new_gain)
    ...
    def forward_pass(self):
        ...
        for rnn_input in rnn_inputs:
            ...
            rec_gain = tf.matmul()
            activation = 1 / (1 + tf.exp(-1 * tf.multiply(gain, state)))
            output = self.output_timestep(state)

            rnn_inputs_edit.append(this_input)
            rnn_states.append(state)
            gain_outputs.append(gain)
        ...
        return (
            ...
            tf.transpose(a=gain_outputs, perm=[1, 0, 2]),
        )
    ...
    def build(self):
        ...
        # --------------------------------------------------
        self.predictions, self.states, self.inputs, self.gains = self.forward_pass()
    ...
    def test(self, trial_batch):
        ...
        # --------------------------------------------------
        outputs, states, inputs, gains = self.sess.run(
            [self.predictions, self.states, self.inputs, self.gains],
            feed_dict={self.x: trial_batch},
        )

        return outputs, states, inputs, gains
