...
Tian Wang Feb 26th, 2022 

Add another input called gain, 3 inputs total
"""
...
class Checkerboard2AFC(Task):
    ...
    def generate_trial_params(self, batch, trial):
        ...
        ################################################## Tian added this
        params["g_bound"] = self.g_bound
        ##################################################
        return params

    def trial_function(self, t, params, gain, RT):
        ...
        if t > target_onset + checker_onset:
            x_t[0:2] = (params["noise"] ** 2) * np.sqrt(self.dt) * np.random.randn(2)
        ...
        if t > target_onset + checker_onset + RT:
    ...
    def generate_trial(self, params):
        ...
        ################################################## Tian added this
        gain = np.random.uniform(params["g_bound"][0], params["g_bound"][1])
        mu = (1200 - 900*gain)
        RT = int(np.round(abs(np.random.normal(mu, 10, 1))))
        ##################################################
        ...
        for t in range(self.N_steps):
            x_data[t, :], y_data[t, :], mask[t, :] = self.trial_function(t * self.dt, params, gain, RT)

        return x_data, y_data, mask, gain, RT
    ...
    def batch_generator(self):
        ...
        while batch > 0:
            ...
            for trial in range(self.N_batch):
                ...
                x,y,m,g,rt = self.generate_trial(p)
                x_data.append(x)
                y_data.append(y)
                mask.append(m)
                params.append(p)
            ...
            yield np.array(x_data), np.array(y_data), np.array(mask), np.array(params), np.array(g_data), np.array(rt_data)
